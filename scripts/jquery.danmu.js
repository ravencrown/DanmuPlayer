/**
 * 专为HDDanmuPlayer定制的jquery.danmu.js
 *
 *
...
(function($) {
    var Danmu = function(element, options) {
        ...
        $("<div class='danmukuTimer'></div>").appendTo(this.$element);

        $(".danmukuTimer").data("nowTime", 0);

        var chatArea = document.createElement("ul");
        chatArea.classList.add("chat-area");

        var chatBox = $("<div class='chatBox'></div>");
        var showArea = $("<div class='showArea'></div>");

        showArea.append(chatArea);

        chatBox.append(showArea);

        $(element).append(chatBox);

        // $(element).append(chatArea);

        this.$timer = $(".danmukuTimer");
        this.$timer.timer({
            delay: 60,
            ...
            callback: function(index) {
                setTimeout(function() {
                    ...
                    $(".danmukuTimer").data("nowTime", $(".danmukuTimer").data("nowTime") + 1);
                    ...
                    if ($(element).data("danmuList")[$(element).data("nowTime")] && me.launched.indexOf($(element).data("nowTime")) < 0) {
                        ...
                        for (var i = (danmus.length - 1); i >= 0; i--) {
                            ...
                            var a_danmu = document.createElement("li");
                            a_danmu.classList.add("danmaku");
                            a_danmu.setAttribute("id", me.id + "tempDanmaku");
                            // a_danmu.addClass('danmaku').attr("id", me.id + "tempDanmaku");

                            var nickName = document.createElement("span");
                            nickName.classList.add("danmu-nickname");

                            var chatText = document.createElement("span");
                            chatText.classList.add("danmu-chatText");

                            a_danmu.appendChild(nickName);
                            a_danmu.appendChild(chatText);
                            chatArea.appendChild(a_danmu);

                            $("#" + me.id + "tempDanmaku .danmu-chatText").text(danmaku.text)
                                .css({
                                    "color": "#fff"
                            ...
                            if (danmaku.nickname.indexOf(":") < 0) {
                                danmaku.nickname = danmaku.nickname + ":";
                            }

                            $("#" + me.id + "tempDanmaku .danmu-nickname").text(danmaku.nickname)
                                .css({
                                    "color": "#ecbb0d",
                                    "-moz-opacity": $(element).data("opacity"),
                                    "opacity": $(element).data("opacity"),
                            ...
                            if (danmaku.position == 0) {
                                ...
                                if (nowCount <= maxCount && nowSecCount <= maxCountPerSec) {
                                    ...
                                    danmaku["row"] = row;
                                    var top_local = (row) * options.FontSizeBig;
                                    danmaku["width"] = $("#" + flyTmpName).height();
                                    var left_local = $("#" + me.id).width();
                                    var newSpeed = ($(element).width() + 400) / me.speed;
                                    nowCount++;
                                    nowSecCount++;

                                    var delay = danmaku["time"];
                                    var speed = 10;
                                    var ispanHeight = 34;
                                    var chatSpan = $("#" + flyTmpName);
